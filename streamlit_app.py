...
# Constants
API_URL = "http://localhost:8000/api/audit"  # Adjust port if needed
...
if submit_button:
    if not url_input:
        ...
        with st.spinner("Running audit... This may take a few minutes as the AI agents analyze the site."):
            try:
                ...
                if result.get("success"):
                    ...
                    if metrics:
                        ...
                        with col1:
                            st.markdown(f'<div class="metric-box"><div class="metric-title">Word Count</div><div class="metric-value">{metrics.get("total_word_count", 0)}</div></div>', unsafe_allow_html=True)
                            st.markdown(f'<div class="metric-box"><div class="metric-title">Images</div><div class="metric-value">{metrics.get("image_count", 0)}</div></div>', unsafe_allow_html=True)
                            
                        with col2:
                            st.markdown(f'<div class="metric-box"><div class="metric-title">CTAs</div><div class="metric-value">{metrics.get("cta_count", 0)}</div></div>', unsafe_allow_html=True)
                            st.markdown(f'<div class="metric-box"><div class="metric-title">Missing Alt Text</div><div class="metric-value">{metrics.get("images_missing_alt_text_pct", 0)}%</div></div>', unsafe_allow_html=True)
                            
                        with col3:
                            headings = metrics.get("heading_counts", {})
                            st.markdown(f'<div class="metric-box"><div class="metric-title">H1 Tags</div><div class="metric-value">{headings.get("h1", 0)}</div></div>', unsafe_allow_html=True)
                            st.markdown(f'<div class="metric-box"><div class="metric-title">H2 Tags</div><div class="metric-value">{headings.get("h2", 0)}</div></div>', unsafe_allow_html=True)
                            
                        with col4:
                            links = metrics.get("link_counts", {})
                            st.markdown(f'<div class="metric-box"><div class="metric-title">Internal Links</div><div class="metric-value">{links.get("internal", 0)}</div></div>', unsafe_allow_html=True)
                            st.markdown(f'<div class="metric-box"><div class="metric-title">External Links</div><div class="metric-value">{links.get("external", 0)}</div></div>', unsafe_allow_html=True)
                    ...
                    if kpis:
                        kpi_data = {
                            "KPI": ["SEO Score", "Links Score", "Usability Score", "Social Score"],
                            "Score": [
                        ...
                        fig = px.bar(
                            ...
                            range_y=[0, 100],
                            text="Score",
                            color_discrete_sequence=["#4da6ff", "#ff9f36", "#17b877", "#faca2b"]
                        )
